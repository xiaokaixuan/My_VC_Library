...
#endif // SQLITE_WRAPPER_REPORT_COLUMN_NAMES
...
bool SQLiteStatement::Execute()
{
	...
	if (rc == SQLITE_BUSY)
	{
		::MessageBoxA(0, "SQLITE_BUSY", 0, 0);
		return false;
	}
	if (rc == SQLITE_ERROR)
	{
		::MessageBoxA(0, "SQLITE_ERROR", 0, 0);
		return false;
	}
	if (rc == SQLITE_MISUSE)
	{
...
bool SQLiteStatement::NextRow()
{
	...
	{
		::MessageBoxA(0, "SQLiteStatement::NextRow SQLITE_MISUSE", 0, 0);
	} 
	else if (rc == SQLITE_BUSY)
	{
		::MessageBoxA(0, "SQLiteStatement::NextRow SQLITE_BUSY", 0, 0);
	} 
	else if (rc == SQLITE_ERROR)
	{
		::MessageBoxA(0, "SQLiteStatement::NextRow SQLITE_ERROR", 0, 0);
	}
	return false;
}
